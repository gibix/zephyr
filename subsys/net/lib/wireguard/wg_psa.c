					  ...
					  uint8_t *public_key)
{
	psa_status_t status;
	psa_key_attributes_t attr = PSA_KEY_ATTRIBUTES_INIT;
	uint8_t private_key[WG_PSA_KEY_SIZE];
	size_t output_len;
	...
	status = psa_generate_random(private_key, WG_PSA_KEY_SIZE);
	if (status != PSA_SUCCESS) {
		NET_DBG("Failed to generate random bytes: %d", status);
		return -EIO;
	}
	...
	if (status != PSA_SUCCESS) {
		NET_DBG("Failed to import X25519 keypair: %d", status);
		return -EINVAL;
	}
			     ...
			     psa_key_id_t key_id)
{
	...
	if (output_len != key_data_len) {
		NET_DBG("Unexpected key size: %zu vs %zu", output_len, key_data_len);
		return -EINVAL;
	}
...
static void wg_psa_random(uint8_t *buf, size_t len)
{
	psa_status_t status;
